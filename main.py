...
def register():
    ...
    if form.validate_on_submit():
        ...
        exists = db.session.scalar(
            db.select(db.exists().where(UserTable.email == email))
        )

        if exists:
            flash("You are already signed up with this mail, try login instead.")
...
def login():
    ...
    if form.validate_on_submit():
        user = db.session.scalar(
            db.select(UserTable).where(UserTable.email == form.user.data)
        )
        if not user:
...
def contact():
    ...
    if request.method == "POST":
        ...
        data = f"{name} Successfully sent your message"
